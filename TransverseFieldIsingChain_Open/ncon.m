...
% 
% Supports disjoint networks, trivial (dimension 1) indices, 1D objects,
% traces, and outer products (both through the zero-in-sequence notation
% and through labelling an implicit trailing index of dimension 1).
...
if ~exist('finalOrder','var')
  % finalOrder not specified - use default: 
  % Negative indices in descending order, consecutive and starting from -1
  finalOrder = [];
end
...
while numel(tensorList)>1 || any(legLinks{1}>0)
  % Ensure contraction sequence is not empty - 
  % converts implicit outer products into zeros-in-sequence outer products
  if isempty(sequence)
    sequence = zeros(1,numel(tensorList)-1);
  end
  % Check first entry in contraction sequence
  if sequence(1)==0
    % It's a zero: Perform an outer product according to the rules of 
    % zeros-in-sequence notation and update contraction sequence
    [tensorList,legLinks,sequence,warnedLegs] = ...
      zisOuterProduct(tensorList,legLinks,sequence,warnedLegs);
  else
    ...
    if tensors(2)==0
      ...
      tracedIndices = tracedIndices([tracedIndices(1:end-1)==...
        tracedIndices(2:end) false]);
      % Check which traced indices appear in sequence. Update contraction list.
      [doingTraces,sequence] = findInSequence(tracedIndices,sequence,...
        tensorList,legLinks,tensors);
      if ~isequal(sort(doingTraces),sort(tracedIndices))
        warnedLegs = warn_suboptimal(doingTraces,tracedIndices,0,warnedLegs,...
          legLinks{tensors(1)},[size(tensorList{tensors(1)}),...
          ones(1,numel(legLinks{tensors(1)})-ndims(tensorList{tensors(1)}))]);
      end
      % Perform traces
      tensorList{tensors(1)} = doTrace(tensorList{tensors(1)},...
        legLinks{tensors(1)},doingTraces);
      % Update leg list
      ...
      % Check which contracted indices appear in sequence. Update list.
      [contractionIndices,sequence] = findInSequence(commonIndices,sequence,...
        tensorList,legLinks,tensors);
      if ~isequal(sort(contractionIndices),sort(commonIndices))
        tdims = [size(tensorList{tensors(1)}),...
          ones(1,numel(legLinks{tensors(1)})-ndims(tensorList{tensors(1)}))];
        tdims = tdims(1:numel(legLinks{tensors(1)}));
        tdims = [tdims size(tensorList{tensors(2)}),...
          ones(1,numel(legLinks{tensors(2)})-ndims(tensorList{tensors(2)}))]; 
        warnedLegs = warn_suboptimal(contractionIndices,commonIndices,1,...
          warnedLegs,[legLinks{tensors(1)} legLinks{tensors(2)}],tdims);
      end
      % Are there any (non-trivial) traced indices on either of these tensors? 
      % If so, warn sequence is suboptimal
      ...
      if ~isempty([traces1 traces2])
        ...
        tdims = [tdims size(tensorList{tensors(2)}),...
          ones(1,numel(legLinks{tensors(2)})-ndims(tensorList{tensors(2)}))]; 
        warnedLegs = warn_suboptimal(contractionIndices,[traces1 traces2],...
          2,warnedLegs,[legLinks{tensors(1)} legLinks{tensors(2)}],tdims);
      end
      % Contract over these indices and update leg list
      [tensorList{tensors(1)},legLinks{tensors(1)}] = ...
        tcontract(tensorList{tensors(1)},tensorList{tensors(2)},...
        legLinks{tensors(1)},legLinks{tensors(2)},contractionIndices);
...
function [rtnIndices,sequence] =...
  findInSequence(indices,sequence,tensorList,legLinks,tensors)
% Check how many of the supplied indices appear at the beginning of "sequence"
ptr = 1;
while ptr<=numel(sequence) && any(indices==sequence(ptr))
  ptr = ptr + 1;
end
rtnIndices = sequence(1:ptr-1);
% If not contracting all possible indices at once, warn sequence is suboptimal
% - remove uncontracted trivial indices from comparison list as are unimportant
for a=numel(indices):-1:1
  if ~any(rtnIndices==indices(a)) && size(tensorList{tensors(1)},...
      find(legLinks{tensors(1)}==indices(a),1))==1
    indices(a) = []; % Trivial so postponing it is not a concern
  end
end
...
% Trace over all indices listed in tracedIndices
sz = size(A);
sz = [sz ones(1,numel(legLabels)-numel(sz))];
tpos = [];
% Find positions of tracing indices
for a=1:numel(tracedIndices)
  tpos = [tpos find(legLabels==tracedIndices(a))]; %#ok
end
% Reorder list of index positions so that they occur in equivalent blocks
...
A = reshape(permute(A,[ind tpos]),prod(sz(ind)),sztrace,sztrace); 
B = 0;
% Perform trace
for a=1:sztrace
...
% Return uncontracted index list. Uncontracted legs are in order 
legs = [legs1(posFreeLegs1) legs2(posFreeLegs2)];
end

function warnedLegs = ...
  warn_suboptimal(doing,couldDo,mode,warnedLegs,legList,legDims)
...
% Mode 2: Contracting two tensors, one carries unevaluated traced index 

% Let couldDo be list of indices which should be contracted but which weren't
for a=1:numel(doing)
...
if ~isempty(couldDo)
  if mode == 2
    ...
    if ~isempty(doing)
      ...
      t = 'Sequence suboptimal: Please contract ind after outer product';
      if numel(couldDo)==1
        t = [t, 'ex ', num2str(couldDo),...
          '. This index connects the same two tensors and is non-trivial.'];
      else
        t = [t, 'ices ', num2str(couldDo),...
          '. These indices connect the same two tensors and are non-trivial.'];
      end
    end
  end
...
function [sequence,legLinks] =...
  checkInputs(tensorList,legLinks,finalOrder,sequence)
% Checks format of input data and returns lists of positive and negative indices
...
if isequal(ncon_skipCheckInputs,true)
  ...
  for a=1:numel(legLinks)
    if size(legLinks{a},1)~=1 || size(legLinks{a},2)~=numel(legLinks{a})
      if isempty(legLinks{a})
        ...
        error(['Link entry ' num2str(a) ' has wrong dimension, should be 1xn']);
      end
    end
    ...
    if numel(legLinks{a}) < numel(tsize)
      if numel(legLinks{a})==1
        error(['Link entry ',num2str(a),' is too short: Tensor size is [',...
          num2str(size(tensorList{a})), '] and legLinks{', num2str(a),...
          '} has only ', num2str(numel(legLinks{a})), ' entry.']);
      else
        error(['Leg link entry ', num2str(a),...
          ' is too short: Tensor size is [', num2str(size(tensorList{a})),...
          '] and legLinks{' num2str(a) '} has only ',...
          num2str(numel(legLinks{a})), ' entries.']);
      end
    end
  end
  ...
  flags = (posindices(1:2:numel(posindices))-...
    posindices(2:2:numel(posindices)))~=0;
  if any(flags)
    errorpos = 2*find(flags~=0,1,'first')-1;
    if errorpos>1 && posindices(errorpos-1)==posindices(errorpos)
      error(['Error in index list: Index ', num2str(posindices(errorpos)),...
  ...
  if exist('maxposindex','var')
    if isempty(posindices)
      error(['Error in index list: Index ', num2str(maxposindex),...
        ' only appears once']);
    end
  ...
  if any(flags)
    ...
    error(['Error in index list: Index ', num2str(altposindices(errorpos)),...
      ' appears more than twice']);
  end
  ...
  % Check negative indices are unique and consecutive, 
  % or unique and correspond to entries in finalOrder
  ...
  if isempty(finalOrder)
    if ~isequal(negindices,-1:-1:-numel(negindices))
      error('Negative indices must be consecutive starting from -1');
    end
  else
  ...
  if exist('sequence','var')
    % Check sequence is a row vector of positive real integers, 
    % each occurring only once and they match the positive leg labels.
    if any(uint32(sequence)~=sequence)
      error('All entries in sequence must be real positive integers or zero');
    end
    if numel(altposindices)~=sum(sequence>0)
      error('Each positive index must appear only once in the sequence,\n%s',...
        'and each index in the sequence must appear on the tensors.');
    end
...
function [tensorList,legLinks,sequence,warnedLegs] =...
  zisOuterProduct(tensorList,legLinks,sequence,warnedLegs)
% This function provides support for the zeros-in-sequence notation 
% described in arXiv:1304.6112

% Perform one or more outer products described by zeros in the sequence

if all(sequence==0) % Final outer product of all remaining objects 
  if numel(sequence) < numel(legLinks)-1
    sequence = zeros(1,numel(legLinks)-1);
    warning('ncon:zisShortSequence','Zeros-in-sequence notation used,\n%s',...
      'and insufficient zeros provided to describe final tensor contraction.');
  end
end
...
if numOPs == numel(legLinks)-1
  ...
  % For OP of n tensors (n=numOPs+1) when more than n tensors remain, proceed 
  % past the zeros in the sequence and read nonzero indices until n+1 tensors 
  % accounted for. Failure to find n+1 tensors implies an invalid sequence.
  ...
  while sum(flags) < numOPs+2
    ...
    if ptr > numel(sequence)
      t = 'Sequence is inconsistent with rules of sequence notation. After a ';
      if numOPs==1
        t = [t 'zero']; %#ok
      else
        t = [t 'string of ' num2str(numOPs) ' zeros']; %#ok
      end
      error([t, ', while reading further indices to identify the ',...
        num2str(numOPs+1), ' tensors involved in the outer product, '...
        'ncon encountered end of index list before identifying all tensors.']);
    end
    if sequence(ptr)==0
      t = 'Sequence is inconsistent with rules of sequence notation. After a ';
      if numOPs==1
      ...
      error([t, ', while reading further indices to identify the ',...
        num2str(numOPs+1) ' tensors involved in the outer product, '...
        'ncon encountered another zero before identifying all tensors.']);
    end
    count = 0;
    for a=1:numel(legLinks)
    ...
    if count~=2
      ...
      error([t, ', while reading further indices to identify the ',...
        num2str(numOPs+1) ' tensors involved in the outer product, ',...
        'ncon encountered an index ', num2str(sequence(ptr)),...
        ' which appears on ', num2str(count),...
        ' tensor(s). Index should appear on exactly 2 tensors at this time.']);
    end
    ptr = ptr + 1;
  end
  % Identify which of these tensors is _not_ participating in the OP 
  % (but is instead contracted with the result of the OP), and unflag it.
  % - Identify the two tensors on which the first nonzero index appears
  % - Examine consecutive nonzero indices until one matches only one of the 
  % two tensors. This is the tensor to unflag.
  firsttensors = [0 0];
  ptr = numOPs+1;
  for a=1:numel(legLinks)
    if any(legLinks{a}==sequence(ptr))
  ...
  OPlist = find(flags);
  % - Check contraction with postOPtensor is over all non-trivial indices
    ...
    if ~any(legLinks{postOPtensor}==OPindices(a))
      ...
      for b=1:numel(OPlist)
        if any(legLinks{b}==OPindices(a))
          isnontriv = size(tensorList{b},legLinks{b}(find(legLinks{b}==...
            OPindices(a),1)))~=1;
          break;
        end
      end
      if isnontriv
        error(['Contraction sequence includes zeros and is inconsistent ',...
          'with rules of zeros-in-sequence notation. After using zeros to ',...
          'contract a group of tensors, all non-trivial indices on those ',...
          'tensors must be contracted with the next object. Contraction ',...
          'did not include index ' num2str(OPindices(a)) '.']);
      end
    end
  end
end
...
while numel(OPsizes)>1
  ...
  for a=numel(commonIndices):-1:1
    if ~any(legLinks{OPlist(ix(2))}==commonIndices(a))
      ...
      if size(tensorList{OPlist(ix(1))},find(legLinks{OPlist(ix(1))}==...
          commonIndices(a),1))==1
        commonIndices(a) = [];
      end
    end
  ...
  if ~isempty(commonIndices)
    ...
    tdims = [size(tensorList{OPlist(ix(1))}),...
      ones(1,numel(legLinks{OPlist(ix(1))})-ndims(tensorList{OPlist(ix(1))}))];
    tdims = tdims(1:numel(legLinks{OPlist(ix(1))}));
    tdims = [tdims size(tensorList{OPlist(ix(2))}),...
      ones(1,numel(legLinks{OPlist(ix(2))})-ndims(tensorList{OPlist(ix(2))}))]; 
    warnedLegs = warn_suboptimal([],commonIndices,1,warnedLegs,...
      [legLinks{OPlist(ix(1))} legLinks{OPlist(ix(2))}],tdims);
  end
  % Contract them
  [tensorList{OPlist(ix(1))},legLinks{OPlist(ix(1))}] = ...
    tcontract(tensorList{OPlist(ix(1))},tensorList{OPlist(ix(2))},...
    legLinks{OPlist(ix(1))},legLinks{OPlist(ix(2))},[]);
