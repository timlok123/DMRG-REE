...
M = zeros(4, 4, 2, 2);
M(1, 1, :, :) = sI;         % Propagate identity
M(1, 2, :, :) = h * sX;     % Transverse field term
M(1, 3, :, :) = sZ;         % Start ZZ interaction
M(2, 4, :, :) = sZ;         % End ZZ interaction
M(3, 4, :, :) = -J * sI;    % Coupling term
M(4, 4, :, :) = sI;         % Propagate identity

% Define left and right boundary tensors
ML = reshape([1; 0; 0; 0], [4, 1, 1]); % Left boundary tensor
MR = reshape([0; 0; 0; J], [4, 1, 1]); % Right boundary tensor (PBC, long-range interaction)
...
E_gs = Ekeep1(end);
disp(['Ground State Energy (16): ', num2str(E_gs)]);

E_gs = Ekeep2(end);
disp(['Ground State Energy (64): ', num2str(E_gs)]);
